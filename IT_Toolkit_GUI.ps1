...
$script:RemoteUrl = "https://raw.githubusercontent.com/sumitguptaslg2004-sudo/IT_TOOLKIT/main/IT_Toolkit_GUI.ps1"
...
$script:LogsDir = "$env:USERPROFILE\Desktop\IT Toolkit Logs"
        ...
        function Test-CommonPorts {
            ...
            foreach ($port in $ports.Keys) {
                try {
                    ...
                    if ($iar.AsyncWaitHandle.WaitOne(120) -and $tcp.Connected) { $open += $ports[$port] }
                    $tcp.Close()
                } catch {}
            }
            return ($open -join ", ")
...
New-ToolButton $tabAdv "Group Policy Update + Report" {
    Start-ToolJob "Group Policy Update" {
        param($logsDir)
        gpupdate /force 2>&1 | Out-String -Stream
        if (-not (Test-Path -LiteralPath $logsDir)) { New-Item -Path $logsDir -ItemType Directory -Force | Out-Null }
        ...
        Start-ToolJob "Add User: $($info.Username)" {
            ...
            try {
                ...
                if ($remoteDesktop) {
                    ...
                    } catch { "NOTE: could not add '$n' to Remote Desktop Users: $($_.Exception.Message)" }
                }
                if ($backupOp) {
                    try {
                        Add-LocalGroupMember -Group "Backup Operators" -Member $n -ErrorAction Stop
                        "Added '$n' to Backup Operators."
                    } catch { "NOTE: could not add '$n' to Backup Operators: $($_.Exception.Message)" }
                }
                if ($powerUser) {
                    try {
                        Add-LocalGroupMember -Group "Power Users" -Member $n -ErrorAction Stop
                        "Added '$n' to Power Users."
                    } catch { "NOTE: could not add '$n' to Power Users: $($_.Exception.Message)" }
                }
...
$PortableToolsDir = if ($ScriptDir) {
    ...
    "$env:USERPROFILE\Desktop\PortableTools"
}
...
New-ToolButton $tabDiag "BatteryInfoView" {
    Launch-OrDownload "BatteryInfoView" "batteryinfoview" @() "BatteryInfoView.exe" "https://www.nirsoft.net/utils/battery_information_view.html"
} -Tooltip "Shows laptop battery health, wear level, and charge cycle info."
New-ToolButton $tabDiag "BlueScreenView" {
    Launch-OrDownload "BlueScreenView" "bluescreenview" @() "BlueScreenView.exe" "https://www.nirsoft.net/utils/blue_screen_view.html"
